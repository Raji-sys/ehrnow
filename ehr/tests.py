...
class PatientFolderViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client.login(username='testuser', password='12345')
        self.url = reverse('patient_folder', args=[self.patient.file_no])
    ...
    def test_get_request_with_permission(self):
        ...
        self.assertIn('clinical_notes', response.context)
        self.assertIn('prescribed_drugs', response.context)
        self.assertIn('hematology_results', response.context)
        self.assertIn('chempath_results', response.context)
        self.assertIn('micro_results', response.context)
        self.assertIn('serology_results', response.context)
        self.assertIn('general_results', response.context)
        self.assertIn('radiology_files', response.context)
        self.assertIn('admission_info', response.context)
        self.assertIn('ward_vital_signs', response.context)
        self.assertIn('ward_medication', response.context)
        self.assertIn('ward_clinical_notes', response.context)
        self.assertIn('theatre_bookings', response.context)
        self.assertIn('theatre_notes', response.context)
        self.assertIn('surgery_bill', response.context)
        self.assertIn('paid_transactions_count', response.context)
...
class FollowUpVisitCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client.login(username='testuser', password='12345')
        self.url = reverse('follow_up_visit_create', args=[self.patient.file_no])
    ...
    def test_post_request(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('medical_record'))
        self.assertTrue(FollowUpVisit.objects.filter(patient=self.patient, clinic='A & E').exists())
        self.assertTrue(PatientHandover.objects.filter(patient=self.patient, clinic='A & E').exists())


class FollowUpListViewTest(TestCase):

    def setUp(self):
        self.client = Client()
        self.url = reverse('follow_up_list')
        self.patient1 = PatientData.objects.create(
            title='Mr.',
            last_name='Doe',
            first_name='John',
    ...
    def test_get_request(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'ehr/record/follow_up_list.html')
        self.assertIn('patients', response.context)
        self.assertIn(self.patient1, response.context['patients'])
        self.assertIn(self.patient2, response.context['patients'])
        self.assertIn('patientFilter', response.context)
